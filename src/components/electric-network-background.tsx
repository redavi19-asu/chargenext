...
function useLightweightNetworkMode() {
...
export function ElectricNetworkBackground() {
  ...
  const lightweight = useLightweightNetworkMode();
  const visiblePaths = lightweight ? networkPaths.slice(0, 5) : networkPaths;
  const visibleNodes = lightweight ? nodePoints.slice(0, 16) : nodePoints;
  const particleCount = lightweight ? 12 : 28;
  const glowFilter = lightweight ? undefined : "url(#strongGlow)";
  const softGlowFilter = lightweight ? undefined : "url(#softGlow)";

  return (
    <div className="chargenext-network-root" aria-hidden="true">
      <div className="chargenext-network-sky" />
      <div
        className="chargenext-network-clouds"
        style={
          lightweight
            ? { filter: "none", animation: "none", opacity: 0.48 }
            : undefined
        }
      />
      {!lightweight && (
      ...
      <div
        className="chargenext-network-horizon-glow"
        style={
          lightweight
            ? { filter: "none", animation: "none", opacity: 0.65 }
            : undefined
        }
      />

      <svg
        className="chargenext-network-scene"
        viewBox="0 0 1600 900"
        preserveAspectRatio="xMidYMid slice"
        style={
          lightweight
            ? { left: "-16%", width: "132%", opacity: 0.86 }
            : undefined
        }
      >
        <defs>
        ...
        <g
          className="chargenext-network-skyline"
          fill="#020817"
          stroke="#075985"
          strokeOpacity="0.36"
          style={lightweight ? { filter: "none" } : undefined}
        >
          ...
          {Array.from({ length: lightweight ? 18 : 34 }).map((_, i) => (
            <rect
              key={i}
              x={35 + i * 46}
              y={292 + (i % 4) * 17}
        ...
        <g className="chargenext-network-grid" stroke="url(#gridFade)" fill="none">
          {Array.from({ length: lightweight ? 11 : 19 }).map((_, i) => {
            const center = lightweight ? 5 : 9;
            const x = 800 + (i - center) * (lightweight ? 125 : 86);
            return (
              <path
                key={`v-${i}`}
                d={`M800 365 L${x} 900`}
          ...
          {(lightweight ? [405, 490, 600, 740, 825] : [405, 445, 490, 542, 600, 665, 740, 825]).map(
            (y, i) => (
              <path
                key={`h-${y}`}
                d={`M${670 - i * 95} ${y} L${930 + i * 95} ${y}`}
          ...
          {visiblePaths.map((path, i) => (
            <g key={path}>
              <path
                d={path}
                stroke="#0284c7"
                strokeOpacity={lightweight ? "0.28" : "0.2"}
                strokeWidth={lightweight ? "9" : "14"}
                filter={glowFilter}
              />
              <path
                d={path}
              ...
              {!reducedMotion && (
                <path
                  ...
                  stroke="#e0f2fe"
                  strokeWidth={lightweight ? "3" : "4"}
                  strokeDasharray={lightweight ? "4 220" : "3 180"}
                  fill="none"
                  filter={glowFilter}
                />
              )}
        ...
        <g className="chargenext-network-nodes">
          {visibleNodes.map(([x, y], i) => (
            <g
              key={`${x}-${y}`}
              className={`chargenext-network-node chargenext-network-node-${(i % 4) + 1}`}
            >
              <circle
                cx={x}
                cy={y}
                r={i % 5 === 0 ? (lightweight ? 32 : 42) : lightweight ? 20 : 26}
                fill="url(#nodeRing)"
              />
              <circle
                cx={x}
        ...
        {!reducedMotion && (
          <g
            className="chargenext-network-powerup"
            fill="none"
            stroke="#f0f9ff"
            strokeWidth={lightweight ? "4" : "6"}
            strokeLinecap="round"
            filter={glowFilter}
          >
        ...
        {Array.from({ length: particleCount }).map((_, i) => (
          <span
            ...
            style={{
              ...
              left: `${(i * 37) % 100}%`,
              top: `${32 + ((i * 29) % 60)}%`,
              boxShadow: lightweight ? "0 0 4px #38bdf8" : undefined,
            } as CSSProperties}
          />
        ))}
      </div>
